...
std::string sha1_string(std::string&& str) {
    unsigned char hash[SHA_DIGEST_LENGTH];
    SHA_CTX sha;
    SHA1_Init(&sha);
    SHA1_Update(&sha, str.c_str(), str.length());
    SHA1_Final(hash, &sha);
...
std::string sha224_string(const std::string&& str) {

    unsigned char hash[SHA224_DIGEST_LENGTH];
    SHA256_CTX sha256;
    SHA224_Init(&sha256);
    SHA224_Update(&sha256, str.c_str(), str.size());
...
std::string sha384_string(const std::string&& str) {

    unsigned char hash[SHA384_DIGEST_LENGTH];
    SHA512_CTX sha384;
    SHA384_Init(&sha384);
    SHA384_Update(&sha384, str.c_str(), str.size());
...
std::string sha512_string(const std::string&& str) {
