...
inline void* create_win32_temp_file_impl()
{
	...
	::fast_io::freestanding::array<char_type,300> arr;
	std::uint32_t temp_path_size;
	if constexpr(is_nt)
	{
		temp_path_size=::fast_io::win32::GetTempPathW(261,arr.data());
	}
	else
	{
		temp_path_size=::fast_io::win32::GetTempPathA(261,arr.data());
	}
	if(temp_path_size==0)
		throw_win32_error();
/*
A dummy implementation before i add uuid for this
*/
	arr[temp_path_size]=0;
	::fast_io::freestanding::array<char_type,1024> res;
	using char_type_may_alias_const_ptr
#if __has_cpp_attribute(gnu::may_alias)
	[[gnu::may_alias]]
#endif
	= char_type const*;
	std::uint32_t get_temp_file_name_ret_code;
	constexpr std::uint32_t zero{};
	if constexpr(is_nt)
	{
		get_temp_file_name_ret_code=::fast_io::win32::GetTempFileNameW(arr.data(),reinterpret_cast<char_type_may_alias_const_ptr>(u""),zero,res.data());
	}
	else
	{
		get_temp_file_name_ret_code=::fast_io::win32::GetTempFileNameA(arr.data(),reinterpret_cast<char_type_may_alias_const_ptr>(u8""),zero,res.data());
	}
	if(get_temp_file_name_ret_code==0u)
		throw_win32_error();
	void* handle;
	if constexpr(is_nt)
	{
		handle=::fast_io::win32::CreateFileW(res.data(),
	...
	{
		handle=::fast_io::win32::CreateFileA(res.data(),
		0x40000000|0x80000000,	//GENERIC_READ|GENERIC_WRITE
		0,		//Prevents other processes from opening a file or device if they request delete, read, or write access.
		nullptr,
		5,		//TRUNCATE_EXISTING
